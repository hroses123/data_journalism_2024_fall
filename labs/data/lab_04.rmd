...
```{r}
# Load data
clean_md_senate_expenses <- read_csv("md_senate_expenses_clean.csv")
...
**A2.**
Ads and fundraising are the highest spending for Hogan. He spends significantly more on ads than anything else. The fundraising is still pretty close to staffing. Alsobrooks spends most on ads and payroll. Both candidates have ads as their highest spending. Alsobrooks spends a significant amount more on payroll than Hogan.
...
**A3.**
It seems that spending has significantly increased from 05-2023. January-May saw the highest spending, perhaps they were preparing for campaigning or beginning to pick up campaigning. 

```{r}
ADS_top <- clean_md_senate_expenses %>%
  filter(PURPOSE_CLEAN_1 == "ADS")

ADS_top_date <- ADS_top %>%
...
**A4.** Alsobrooks has significantly more spending on payroll than Hogan has for staffing -- see memo, which I assume as the payroll-esk. Alsobrooks has 24 different people on payroll whereas Hogan has one person on staffing. 

```{r}
clean_md_senate_expenses |>
  filter (PURPOSE_CLEAN_1 == "PAYROLL" |PURPOSE_CLEAN_1 == "STAFFING - SEE MEMO")  |>
  group_by(candidate) |>
  summarise(
    total=sum(amount)
  ) |>
...
**A5**
An interesting story idea is how Hogan and Alsobrooks spend differently. Hogan focuses more on ads, while Alsobrooks spends more on payroll. This could show different campaign strategies—Hogan relying on media presence and Alsobrooks building a bigger team. It would be interesting to see how these choices affect their campaigns. Also Alsobrooks spends much more on payroll and has significantly more people on her team compared to Hogan, who seems to focus less on staffing or puts it into a different category.
