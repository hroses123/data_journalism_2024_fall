...
md_abortion_data <- read_csv("data/md_abortion_24.csv") %>%
  ...
  mutate(
    geoid = as.character(county),
    total_abortion_votes = for_votes + against_votes,
    abortion_pct_for = (for_votes / total_abortion_votes) * 100
  )
...
md_pres_data <- read_csv("data/md_pres_county_24.csv") %>%
  ...
  mutate(
    geoid = as.character(county),
    total_pres_votes = trump_votes + harris_votes,
    trump_pct = (trump_votes / total_pres_votes) * 100
  )

md_combined_data <- md_abortion_data %>%
  left_join(md_pres_data, by = "geoid") %>%
  mutate(
    pct_diff_abortion_trump = abortion_pct_for - trump_pct
  )
...
ggplot(md_map_data) +
  geom_sf(aes(fill = pct_diff_abortion_trump), color = "white", size = 0.2) +
  scale_fill_gradient2(
    low = "blue", high = "red", mid = "white",
    midpoint = 0, name = "Abortion Vote - Trump % Difference"
  ) +
  labs(
    title = "Maryland County-Level Comparison of Abortion Vote vs. Trump Performance",
    subtitle = "Percentage Difference by County",
    caption = "Source: md_abortion_24.csv, md_pres_county_24.csv"
  ) +
  theme_minimal()
