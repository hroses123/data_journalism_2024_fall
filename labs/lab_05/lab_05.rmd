...
combined_harris <- bind_rows(august_harris, july_harris)
...
```{r}
combined_harris <- combined_harris %>%
  mutate(first_name = str_to_upper(first_name),
         last_name = str_to_upper(last_name),
          address_one = str_to_upper(address_one)
  )

new_voters <- new_voters %>%
  mutate(FirstName = str_to_upper(FirstName),
         LastName = str_to_upper(LastName),
         Address = str_to_upper(Address))

combined_data <- combined_harris %>%
  inner_join(new_voters, join_by(first_name == FirstName, 
                           last_name == LastName, 
                           address_one == Address))
...
A1. 
I used both first and last names to uniquely identify individuals and reduce the chances of matching different people with common names.

I also used Address and address_one because they most closely resembled each other in how they looked. I initially tried with streetname and housename but it was too difficult because they did not resemble each other and I could not combine them. 

Potential problems include the possibility of false positives if multiple people have the same name and address, leading to duplicated records.

Incomplete data, like missing values , could lead to missing records during the join.
...
A2. 
365 donations
270 unique donations
Brittany Musolino, Eric Snope, Sophia Hoffer-Perkins have given the most number of times (7).


```{r}
donations <- combined_data %>%
...
```{r}
party_gender <- combined_data %>%
  group_by(Party, Gender) %>%
  summarise(donation_count = n()) %>%
  arrange(desc(donation_count))

non_dem_donations <- combined_data %>%
  filter(Party == "DEM") %>%
  summarise(non_democrat_count = n())
(party_gender)
(non_dem_donations)
```
...
What is the average donation amount made by voters in the combined dataset?

average_donations <- combined_data %>%
  group_by(first_name, last_name) %>%
  summarize(average_donation = mean))
