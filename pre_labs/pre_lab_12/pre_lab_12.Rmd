...
**Task** Run the following code and describe the dataframe it outputs. **Answer** Same Cardin as last week, but now we have the full text of things and it is not just a url or title, the full press release! Lots of Info!
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It makes it so they only appear once in the list, no matter how many times they appear
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It turns all of the words to lowercase and removes all punctuation, we broke all of the words a part so they are separate, this is all of the words for all of the press releases. One data frame with one column that is every single word, not unique words
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** We see a chart of the most common words, the top are articles mostly which makes sense to me. These words are not substantial. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** We got a list of the words that we do not want to include, we excluded these words through anti_join because these words are not substantial
...
**Task** Run the following code and describe the results. **Answer** We did the anti_join for the two data frames and we see the top word for each is different, which is interesting. Health vs. Baltimore, this lends itself to an interesting story about what was most important to the electorate in that time. There is a good amount of overlap, though, which makes sense because he is still the same person perhaps with the same ideology. 
...
**Task** Run the following code and describe the results. **Answer** These are two word phrases that are used a lot. We made sure these were substantial words. The first is amount included, second health care which seems very important to the electorate over number one. We're getting discrete topics like mental health, more nuance and substantive. Perhaps this could help more with developing a story, but not so much. 
...
**Task** Run the following code and describe the results. **Answer** There is a lot of overlap with the phrases from before, but here I see purpose funds is different, which could lend itself into a story if it is something relavant and interesting. Still, we see chesapeake bay and human rights, whcih I guess are ongoing issues in MD. No more Covid-19, so I guess it is not a ticket issue in 2024. Only took us 4 years to "get over it" ;)
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**  drove vs drive, driven vs. drive
Probably the first because the tenses are more descriptive and everything is not just in present tense, so we can more easily understand when things happened. 
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** The results are pretty similar and seem to just be in a different order. 2022 top was fund and 2024 top was baltimore which could be an interesting comparison. Neither seems to be more helpful than the other. 
...
Critical is not always negative, but I could understand. 
Supreme is positive...kind of confused. 
trump is positive...could be interesting if it is in actuality referring to Donald Trump. 
