...
**Answer** The code  is adding a column on percentages based on already existing columns in decimals, but the one below is different because we multiplied it by 100. Therefore, it is a percentage instead of a decimal. We made it with the code above and added x100.
...
**Answer**
We changed the ordering of this data in descending order, so we see the greatest percentages first. 
...
**Answer**
This is a more useful way of looking at the data because we are better able to see the data and it is not in a mish mosh. We are able to gauge the number of people who vote early or on voting day. It tells us about the trends of the electorate. 
...
**Answer**
str_to_lower(), this converts everything into lower case
str_trim(), this removes spaces before and after the word
...
**Answer**
I think the mutate is creating that new row called amount category, but this varies a lot. So we use a case when which can be seen in the amount rows, which assigns data to categories. The head is there so we can just see a small amount of the data. 
...
**Answer**
It allows us to be more focused in our analysis and not look at too much.
...
We are just changing the way in which the date is shown so we can see the date more accurately and formulated better. It changes it from character type to date which allows us to pick the dates apart. 
...
**Answer**
The month column represents the first day of each month and organizes total_amount monthly. The  increase in spending in June 2022 likely reflects changes in campaigns where political campaigns picked up the pace in campaigning because the election was approaching. 
