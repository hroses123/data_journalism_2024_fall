...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are different variations of the same place and some duplicates.
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It changed all of the lower case W. Millar's to uppercase. It added a period after each W. We went from 5 variations to 2 variations. Additionally, on page 2, a lowercase Walgreens was capitalized. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** It does not seem much different, we still have the same issue with Walters but there are less variations. It makes the Wegman's and We the Pizza the same. 
