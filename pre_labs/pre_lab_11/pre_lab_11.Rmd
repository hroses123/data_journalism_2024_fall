...
We're going to use a library called `rvest`, which you can install it the same way we've done all installs: go to the console and i `install.packages("rvest")`.
...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The columns have really long names and they do not seem concise enough, they also have spaces in between words, additionally we need them to be <num> not <chr>
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** I think they are reffering to the column number

```{r}
# Read in all _ from table, get the HTML table.
results <- pres_url |>
  read_html() |>
  html_table()
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** The output is a list of text content from `<li>` tags, including names of sections or categories such as "Representatives," "Leadership," "Committees," and various topics like "Agriculture," "Appropriations" and more. 
```{r}
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** I see the name of the candidate with a url
