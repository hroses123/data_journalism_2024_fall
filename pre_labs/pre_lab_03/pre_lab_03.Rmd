...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** There were no parsing errors. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** District is <dbl>, precinct is <chr> and mail is <dbl>. 
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: There are no NA values. This implies that the dataset is complete and reliable. 
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** It is interesting to me that there are so many deceased people. This prompts a story of who is voting for them? How are they voting for them? Are people voting for them or did they vote before passing away?
...
**Answer** There could be NA data. There might be incorrect data. There might be duplicates. 
...
**Answer** It changes it to x1_linenumber. 
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** It says there is an error in summarise. I think this means that it could not summarise the data for some reason, perhaps because of an inconsistency in the data names. We tried to sum up a character column, which it cannot do. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** There are 2. Derrick Hamilton. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** This gave me all unique records and just one of each. 
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. 2
